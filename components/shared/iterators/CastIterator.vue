			...
			template(v-slot:default='{ items }')
				masonry.mx-n2(:cols="{ 600: 2, 960: 3, 1265: 4, 1905: 5, default: 6 }" gutter='0')
					div.mx-2.my-4(v-for='person in items' :key='person.credit_id')
						v-card(outlined hover nuxt :to='`/people/${person.id}-${$utils.slugify(String(person.name))}#tab__info`')
							v-card-text.text-center
								v-avatar(size='100')
									...
									v-icon.grey(v-else x-large disabled :class='{ "darken-2": $vuetify.theme.dark, "lighten-3": !$vuetify.theme.dark }') mdi-account
								.title {{person.name}}
