			...
			template(v-slot:append)
				.pa-3(:class='{ "pb-8": $vuetify.breakpoint.smAndDown }')
					v-btn(text block @click='discover' :loading='loading' :color='queried.with_genres != query.with_genres || queried.without_genres != query.without_genres ? "primary" : undefined')
						span Apply
		v-container(fluid)
			media-iterator(ref='mediaIterator' :media='discovery' :loading='loading' flat-header default-view='wide' :title='$vuetify.breakpoint.mdAndUp ? "Results" : ""' :dense='queryDrawer' :resource='["discover", "tv"]' :query='query')
...
<script>
	...
	export default {
		...
		data: ({ $store }) => ({
			...
			genres: $store.state.genres.series.map(g => ({
				id: g.id,
				name: g.name,
				with: false,
				without: false
		...
		methods: {
			async discover() {
				try {
					this.loading = true;
					let response = await this.$api.tmdb(["discover", "tv"], this.query);
